...
router.get('/script', function(req, res, next) {
  var templatePath = path.join(__dirname, '..', 'bloomfilter_script_template_min.js');
  let script = fs.readFileSync(templatePath).toString();
  script = script.replace('[LATEST_BLOOMFILTER_ARRAY]', JSON.stringify([].slice.call(this.bloomFilter.buckets)));
  script = script.replace('[LATEST_BLOOMFILTER_COUNT]', this.bloomFilterCounter);
  res.send(script);
});
